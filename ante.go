...
import (
	...
	dilithium "github.com/cosmos/cosmos-sdk/crypto/dilithium"
)
...
func SigVerificationDecorator(ctx sdk.Context, tx sdk.Tx) sdk.Result {
    stdTx, ok := tx.(auth.StdTx)
    if !ok {
        return sdk.ErrUnknownRequest("Tx must be StdTx").Result()
    }

    for _, sig := range stdTx.Signatures {
        if !dilithium.Verify(sig.PubKey, stdTx.GetSignBytes(), sig.Signature) {
